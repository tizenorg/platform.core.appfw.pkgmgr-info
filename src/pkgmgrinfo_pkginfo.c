...
static gint __list_strcmp(gconstpointer a, gconstpointer b, gpointer user_data)
{
		...
		pkgmgrinfo_filter_x *filter, GList **list)
{
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) {
		...
		if (pkgid != NULL)
			*list = g_list_insert_sorted_with_data(*list, pkgid,
					__list_strcmp, NULL);
	}
		...
		pkgmgrinfo_pkginfo_h *handle)
{
	...
	ret = _pkginfo_get_pkginfo(pkgid, uid,
