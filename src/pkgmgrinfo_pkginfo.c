		...
		const char *locale, int flag, GList **list)
{
	...
	ret = _pkginfo_get_packages(db, locale, filter, flag, list);
	sqlite3_close_v2(db);
		...
		pkgmgrinfo_pkg_list_cb pkg_list_cb, void *user_data)
{
	...
	ret = _pkginfo_get_filtered_list(uid, filter, locale, flag, &list);
		...
		pkgmgr_pkginfo_x **pkginfo)
{
	...
	ret = _pkginfo_get_filtered_list(uid, filter, locale,
...
API int pkgmgrinfo_pkginfo_usr_filter_count(pkgmgrinfo_pkginfo_filter_h handle, int *count, uid_t uid)
{
	...
	ret = _pkginfo_get_filtered_list(uid, (pkgmgrinfo_filter_x *)handle,
			locale, 0, &list);
	if (ret == PMINFO_R_OK && uid != GLOBAL_USER)
		ret = _pkginfo_get_filtered_list(GLOBAL_USER, handle, locale, 0,
				&list);
