...
static int _pkginfo_get_packages(sqlite3 *db, const char *locale,
		pkgmgrinfo_filter_x *filter, GList **packages)
{
	static const char query_raw[] =
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) {
		...
		*packages = g_list_append(*packages, info);
	}
...
static int _pkginfo_get_filtered_list(uid_t uid, pkgmgrinfo_filter_x *filter,
		const char *locale, GList **list)
{
	...
	ret = _pkginfo_get_packages(db, locale, filter, list);
	sqlite3_close_v2(db);
	if (ret != PMINFO_R_OK)
		return PMINFO_R_ERROR;
...
static int _pkginfo_get_filtered_foreach_pkginfo(uid_t uid,
		pkgmgrinfo_filter_x *filter, pkgmgrinfo_pkg_list_cb pkg_list_cb,
		void *user_data)
{
	int ret;
	char *locale;
	...
	ret = _pkginfo_get_filtered_list(uid, filter, locale, &list);
	if (ret == PMINFO_R_OK && uid != GLOBAL_USER)
		ret = _pkginfo_get_filtered_list(GLOBAL_USER, filter, locale,
				&list);
		...
		void *user_data, uid_t uid)
{
	...
	return _pkginfo_get_filtered_foreach_pkginfo(uid, NULL, pkg_list_cb,
			user_data);
}
...
API int pkgmgrinfo_pkginfo_usr_filter_count(pkgmgrinfo_pkginfo_filter_h handle, int *count, uid_t uid)
{
	...
	ret = _pkginfo_get_filtered_list(uid, (pkgmgrinfo_filter_x *)handle,
			locale, &list);
	if (ret == PMINFO_R_OK && uid != GLOBAL_USER)
		ret = _pkginfo_get_filtered_list(GLOBAL_USER, handle, locale,
				&list);
		...
		pkgmgrinfo_pkg_list_cb pkg_cb, void *user_data, uid_t uid)
{
	...
	return _pkginfo_get_filtered_foreach_pkginfo(uid, handle, pkg_cb,
			user_data);
}
