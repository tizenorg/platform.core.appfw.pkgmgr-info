...
static int _appinfo_get_applications(sqlite3 *db, const char *locale,
		pkgmgrinfo_filter_x *filter, int flag, GList **applications)
{
	static const char query_raw[] =
...
static int _appinfo_get_filtered_list(uid_t uid, pkgmgrinfo_filter_x *filter,
		const char *locale, int flag, GList **list)
{
	int ret;
	sqlite3 *db;
	const char *dbpath;
	...
	ret = _appinfo_get_applications(db, locale, filter, flag, list);
	sqlite3_close_v2(db);
		...
		pkgmgr_appinfo_x **appinfo, bool is_disabled)
{
	...
	ret = _appinfo_get_filtered_list(uid, filter, locale, APPINFO_ALL, &list);
	if (!g_list_length(list) && uid != GLOBAL_USER)
		ret = _appinfo_get_filtered_list(GLOBAL_USER, filter, locale, APPINFO_ALL,
				&list);
		...
		void *user_data)
{
	...
	ret = _appinfo_get_filtered_list(uid, filter, locale, flag, &list);
	if (ret == PMINFO_R_OK && uid != GLOBAL_USER)
		ret = _appinfo_get_filtered_list(GLOBAL_USER, filter, locale, flag,
				&list);
		...
		pkgmgrinfo_app_list_cb app_func, void *user_data, uid_t uid)
{
	...
	ret = _appinfo_get_filtered_foreach_appinfo(uid, filter, APPINFO_ALL, app_func,
			user_data);
...
API int pkgmgrinfo_appinfo_get_usr_install_list(pkgmgrinfo_app_list_cb app_func, uid_t uid, void *user_data)
{
	...
	return _appinfo_get_filtered_foreach_appinfo(uid, NULL, APPINFO_ALL, app_func,
			user_data);
}
		...
		void *user_data)
{
	...
	return _appinfo_get_filtered_foreach_appinfo(uid, NULL, flag, app_func,
			user_data);
}
...
API int pkgmgrinfo_appinfo_usr_filter_count(pkgmgrinfo_appinfo_filter_h handle, int *count, uid_t uid)
{
	...
	ret = _appinfo_get_filtered_list(uid, handle, locale, 0, &list);
	if (ret == PMINFO_R_OK && uid != GLOBAL_USER)
		ret = _appinfo_get_filtered_list(GLOBAL_USER, handle, locale, 0,
				&list);
		...
		pkgmgrinfo_app_list_cb app_cb, void *user_data, uid_t uid)
{
	...
	return _appinfo_get_filtered_foreach_appinfo(uid, handle, APPINFO_ALL, app_cb,
			user_data);
}
