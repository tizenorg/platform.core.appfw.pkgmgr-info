...
static int _appinfo_get_applications(uid_t uid, const char *locale,
		...
		pkgmgr_appinfo_x **appinfo)
{
	...
	ret = _appinfo_get_applications(uid, locale, filter,
			PMINFO_APPINFO_GET_ALL, &list);
	if (!g_list_length(list) && uid != GLOBAL_USER)
		ret = _appinfo_get_applications(GLOBAL_USER, locale, filter,
				PMINFO_APPINFO_GET_ALL, &list);
		...
		void *user_data)
{
	...
	ret = _appinfo_get_applications(uid, locale, filter, flag, &list);
...
{
	return pkgmgrinfo_appinfo_get_usr_list(handle, component, app_func,
...
API int pkgmgrinfo_appinfo_get_installed_list(pkgmgrinfo_app_list_cb app_func,
		void *user_data)
{
	return pkgmgrinfo_appinfo_get_usr_installed_list(app_func, GLOBAL_USER,
			user_data);
}
...
API int pkgmgrinfo_appinfo_usr_filter_count(pkgmgrinfo_appinfo_filter_h handle, int *count, uid_t uid)
{
	...
	ret = _appinfo_get_applications(uid, locale, handle, 0, &list);
	if (ret == PMINFO_R_OK && uid != GLOBAL_USER)
		ret = _appinfo_get_applications(GLOBAL_USER, locale, handle, 0,
				&list);
