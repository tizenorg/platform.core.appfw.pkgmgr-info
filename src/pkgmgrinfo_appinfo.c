...
static gpointer __copy_icon(gconstpointer src, gpointer data)
{
	...
	if (tmp->size)
		icon->size = strdup(tmp->resolution);
...
static gpointer __copy_notification(gconstpointer src, gpointer data)
{
	...
	if (tmp->name)
		notification->text = strdup(tmp->name);
...
static gpointer __copy_define(gconstpointer src, gpointer data)
{
	...
	if (tmp->allowed)
		define->allowed = strdup(tmp->allowed);
	if (tmp->request)
		define->request = strdup(tmp->request);
...
static gpointer __copy_datashare(gconstpointer src, gpointer data)
{
	...
	if (tmp->define)
		datashare->define = g_list_copy_deep(tmp->define,
					(GCopyFunc) __copy_define,
					NULL);
	if (tmp->request)
		datashare->request = strdup(tmp->request);
...
static int _appinfo_copy_appinfo(application_x **application, application_x *data)
{
	...
	app_info->label = g_list_copy_deep(data->label, (GCopyFunc) __copy_label, NULL);
	app_info->icon = g_list_copy_deep(data->icon, (GCopyFunc) __copy_icon, NULL);
	app_info->image = g_list_copy_deep(data->image, (GCopyFunc) __copy_image, NULL);
	app_info->category = g_list_copy_deep(data->category, strdup, NULL);
	app_info->metadata = g_list_copy_deep(data->metadata, (GCopyFunc) __copy_metadata, NULL);
	app_info->permission = g_list_copy_deep(data->permission, (GCopyFunc) __copy_permission, NULL);
	app_info->launchconditions = g_list_copy_deep(data->launchconditions, strdup, NULL);
	app_info->notification = g_list_copy_deep(data->notification, (GCopyFunc) __copy_notification, NULL);
	app_info->datashare = g_list_copy_deep(data->datashare, (GCopyFunc) __copy_datashare, NULL);
	app_info->datacontrol = g_list_copy_deep(data->datacontrol, (GCopyFunc) __copy_datacontrol, NULL);
	app_info->background_category = g_list_copy_deep(data->background_category, strdup, NULL);
	app_info->appcontrol = g_list_copy_deep(data->appcontrol, (GCopyFunc) __copy_appcontrol, NULL);
