...
static int _appinfo_add_label_info_into_list(const char *locale,
		char *value, GList **label)
{
	label_x *info;
...
static int _appinfo_add_icon_info_into_list(const char *locale,
		char *value, GList **icon)
{
	icon_x *info;
		...
		GHashTable *applications)
{
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) {
		...
		if (flag & PMINFO_APPINFO_GET_LABEL) {
			...
			if (_appinfo_add_label_info_into_list(locale,
		...
		if (flag & PMINFO_APPINFO_GET_ICON) {
			tmp_record = NULL;
			_save_column_str(stmt, idx++, &tmp_record);
			if (_appinfo_add_icon_info_into_list(locale,
