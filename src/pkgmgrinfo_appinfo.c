...
API int pkgmgrinfo_appinfo_is_splash_screen_display(pkgmgrinfo_appinfo_h handle, bool *splash_screen_display)
{
	pkgmgr_appinfo_x *info = (pkgmgr_appinfo_x *)handle;

	if (info == NULL || info->app_info == NULL || splash_screen_display == NULL) {
		_LOGE("Invalid parameter");
		return PMINFO_R_EINVAL;
	}
