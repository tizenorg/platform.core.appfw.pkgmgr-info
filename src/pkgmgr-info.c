...
#define QUERY_CREATE_VIEW "CREATE temp VIEW '%s' as select * "\
...
static const char *parserdb_tables[] = {
	...
	"package_app_app_share_allowed",
	"package_app_app_app_permission",
	"package_app_app_share_request",
	"package_app_app_svc",
	"package_info",
	"package_app_icon_section_info",
	"package_localized_info",
	"package_app_image_info",
	"package_app_privilege_info",
	NULL
};
...
static int __attach_and_create_view(sqlite3 *handle, const char *db, const char *tables[], uid_t uid)
{
	...
	if(uid != GLOBAL_USER){
		snprintf(query, MAX_QUERY_LEN - 1, QUERY_ATTACH, db);
		if (SQLITE_OK != sqlite3_exec(handle, query, NULL, NULL, &error_message)) {
			_LOGD("Don't execute query = %s error message = %s\n", query, error_message);
			sqlite3_free(error_message);
		}

		for (i = 0; tables[i]; i++) {
			snprintf(query, MAX_QUERY_LEN - 1, QUERY_CREATE_VIEW,
					tables[i], tables[i], tables[i]);
			if (SQLITE_OK != sqlite3_exec(handle, query,
