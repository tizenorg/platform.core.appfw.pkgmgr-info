		...
		pkgmgrinfo_pkg_list_cb pkg_list_cb, void *user_data, uid_t uid)
{
	...
	while (tmp) {
		...
		pkgmgr_parser_free_manifest_xml(tmp->manifest_info);
		free(tmp);
		tmp = next;
	}
		...
		pkgmgr_pkginfo_x **pkginfo)
{
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) {
		...
		idx = 0;
		_save_column_str(stmt, idx++, &mfx->for_all_users);
		_save_column_str(stmt, idx++, &mfx->package);
		_save_column_str(stmt, idx++, &mfx->version);
		_save_column_str(stmt, idx++, &mfx->installlocation);
		_save_column_str(stmt, idx++, &mfx->removable);
		_save_column_str(stmt, idx++, &mfx->preload);
		_save_column_str(stmt, idx++, &mfx->readonly);
		_save_column_str(stmt, idx++, &mfx->update);
		_save_column_str(stmt, idx++, &mfx->appsetting);
		_save_column_str(stmt, idx++, &mfx->system);
		_save_column_str(stmt, idx++, &mfx->type);
		_save_column_str(stmt, idx++, &mfx->package_size);
		_save_column_str(stmt, idx++, &mfx->installed_time);
		_save_column_str(stmt, idx++, &mfx->installed_storage);
		_save_column_str(stmt, idx++, &mfx->storeclient_id);
		_save_column_str(stmt, idx++, &mfx->mainapp_id);
		_save_column_str(stmt, idx++, &mfx->package_url);
		_save_column_str(stmt, idx++, &mfx->root_path);
		_save_column_str(stmt, idx++, &mfx->csc_path);
		_save_column_str(stmt, idx++, &mfx->nodisplay_setting);
		_save_column_str(stmt, idx++, &mfx->api_version);

		if (_pkginfo_get_author(mfx->package, &mfx->author)) {
			pkgmgr_parser_free_manifest_xml(mfx);
			sqlite3_finalize(stmt);
			return PMINFO_R_ERROR;
		}

		if (_pkginfo_get_label(mfx->package, locale, &mfx->label)) {
			pkgmgr_parser_free_manifest_xml(mfx);
			sqlite3_finalize(stmt);
			return PMINFO_R_ERROR;
		}

		if (_pkginfo_get_icon(mfx->package, locale, &mfx->icon)) {
			pkgmgr_parser_free_manifest_xml(mfx);
			sqlite3_finalize(stmt);
			return PMINFO_R_ERROR;
		}

		if (_pkginfo_get_description(mfx->package, locale,
					&mfx->description)) {
			pkgmgr_parser_free_manifest_xml(mfx);
			sqlite3_finalize(stmt);
			return PMINFO_R_ERROR;
		...
		if (_pkginfo_get_privilege(mfx->package, &mfx->privileges)) {
			pkgmgr_parser_free_manifest_xml(mfx);
			sqlite3_finalize(stmt);
			return PMINFO_R_ERROR;
		}
...
API int pkgmgrinfo_appinfo_get_usr_install_list2(pkgmgrinfo_app_list_cb app_func, uid_t uid, void *user_data)
{
...
API int pkgmgrinfo_appinfo_usr_filter_foreach_appinfo2(pkgmgrinfo_appinfo_filter_h handle,
				pkgmgrinfo_app_list_cb app_cb, void *user_data, uid_t uid)
{
	char *syslocale;
	char *locale;
