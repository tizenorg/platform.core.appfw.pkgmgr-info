...
static int __disable_global_app_for_user(const char *appid, uid_t uid)
{
	...
	_LOGD("jungh disable appid[%s], uid[%d]\n", appid, (int)uid);
	snprintf(query, MAX_QUERY_LEN, "INSERT INTO package_app_disable_for_user(app_id, uid) VALUES('%s', '%d')", appid, (int)uid);
...
static int __enable_global_app_for_user(const char *appid, uid_t uid)
{
	...
	_LOGD("jungh enable appid[%s], uid[%d]\n", appid, (int)uid);
	snprintf(query, MAX_QUERY_LEN, "DELETE FROM package_app_disable_for_user WHERE app_id='%s' AND uid='%d'", appid, (int)uid);
	_LOGD("jungh query[%s]\n", query);
	ret = __exec_query(query);
	if (ret == -1)
