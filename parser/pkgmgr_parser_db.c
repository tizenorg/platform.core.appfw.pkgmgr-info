...
static int __insert_pkg_default_values(manifest_x *mfx)
{
	if (mfx == NULL)
	...
	if (!mfx->installlocation)
		mfx->installlocation = strdup("auto");
	...
	if (!mfx->support_disable)
		mfx->support_disable = strdup("false");

	return PMINFO_R_OK;
}

static int __insert_app_default_values(application_x *app)
{
	if (app == NULL)
		return PMINFO_R_EINVAL;
	...
	if (!app->enabled)
		app->enabled = strdup("true");

	if (!app->hwacceleration)
		app->hwacceleration = strdup("use-system-setting");

	if (!app->screenreader)
		app->screenreader = strdup("use-system-setting");

	if (!app->indicatordisplay)
		app->indicatordisplay = strdup("true");

	if (!app->guestmode_visibility)
	...
	if (!app->ui_gadget)
		app->ui_gadget = strdup("false");

	if (!app->splash_screen_display)
		app->splash_screen_display = strdup("true");

	if (!app->launch_mode)
		app->launch_mode= strdup("caller");
...
static int __insert_application_info(manifest_x *mfx)
{
	...
	for (tmp = mfx->application; tmp; tmp = tmp->next) {
		...
		ret = __insert_app_default_values(app);
		if (ret != PMINFO_R_OK) {
			_LOGE("Failed to set default values for application");
			if (type)
				free(type);
			return -1;
		}
		...
		sqlite3_snprintf(MAX_QUERY_LEN, query,
			...
			mfx->preload, app->submode, app->submode_mainid, mfx->installed_storage, app->process_pool,
			...
			effective_appid, app->splash_screen_display);
...
static int __insert_manifest_info_in_db(manifest_x *mfx, uid_t uid)
{
	...
	ret = __insert_pkg_default_values(mfx);
	if (ret != PMINFO_R_OK) {
	...
	sqlite3_snprintf(MAX_QUERY_LEN, query,
		...
		mfx->installlocation, mfx->package_size, mfx->removable, mfx->preload, mfx->readonly,
		mfx->update, mfx->appsetting, mfx->nodisplay_setting, mfx->system, auth_name,
		auth_email, auth_href, mfx->installed_time, mfx->installed_storage,
		mfx->storeclient_id,
		mfx->mainapp_id, mfx->package_url, mfx->root_path, mfx->csc_path, mfx->support_disable);
