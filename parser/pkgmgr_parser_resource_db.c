...
static int __attach_and_create_view(sqlite3 *handle, const char *db, uid_t uid)
{
	int ret = SQLITE_OK;
	char *err;
	char *query;
...
static int __open_parser_db(sqlite3 **parser_db, uid_t uid)
{
	int ret = -1;
	...
	ret = __attach_and_create_view(*parser_db, getUserPkgParserDBPath(), uid);
...
int pkgmgr_parser_resource_db_load(const char *package, resource_data_t **data)
{
	...
	ret = __open_parser_db(&pkginfo, getuid());
	retvm_if(ret != SQLITE_OK, PMINFO_R_ERROR, "connect db [%s] failed!");
