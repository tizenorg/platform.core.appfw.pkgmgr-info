...
/**
 * @fn int pkgmgr_parser_update_global_app_disable_info_in_db(const char *appid, uid_t uid, int is_disable)
 * @brief	This API updates disable info about global app for user specified by uid from db
 *
 * @par		This API is for package-manager installer backends
 * @par Sync (or) Async : Synchronous API
 *
 ...
 * @see		pkgmgr_parser_insert_manifest_info_in_db()
 * @code
static int disable_global_appfor_user(const char *appid, uid_t uid)
{
	int ret = 0;
	ret = pkgmgr_parser_update_global_app_disable_info_in_db(appid, uid, 1);
	if (ret < 0)
		return -1;
	return 0;
}
...
/**
 * @fn int pkgmgr_parser_update_app_disable_info_in_db(const char *appid, uid_t uid, int is_disable);
 * @brief	This API updates disable info about app from db
 *
 * @par		This API is for package-manager installer backends
 * @par Sync (or) Async : Synchronous API
 *
...
{
	int ret = 0;
	ret = pkgmgr_parser_update_app_disable_info_in_db(appid, getuid(), 1);
	if (ret < 0)
		return -1;
	return 0;
}
 * @endcode
 */
int pkgmgr_parser_update_app_disable_info_in_db(const char *appid, uid_t uid, int is_disable);
